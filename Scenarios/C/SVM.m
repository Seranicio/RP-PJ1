...
%       disp('HOG1');
    ...
    % Quick fix because i think it cannot have class '0'.
...
%     'Coding','onevsall'
    ...
    SVMmodel=fitcecoc(PCA_data.X', PCA_data.y','Learners','svm','Coding','onevsone');
    
    [ypred] = predict(SVMmodel, TestingPCA.X');
...
if (SceneryCRunDefault == 1)
    if(HOGType == 1)
        FinalTestingData = [TSDATATesting_HOG_01{:};TSDATATesting_Hue{:};TSDATATesting_Haar{:}];
    elseif(HOGType == 2)
        FinalTestingData = [TSDATATesting_HOG_02{:};TSDATATesting_Hue{:};TSDATATesting_Haar{:}];
    else
        FinalTestingData = [TSDATATesting_HOG_03{:};TSDATATesting_Hue{:};TSDATATesting_Haar{:}];
    end

    SelectedFeaturesPositions = SelectedFeaturesStruct{minimum(1)};

    for j=1:size(FinalTestingData,2)
        for i=1:length(SelectedFeaturesPositions)
            TestingFinalPCA.X(i,j) =  FinalTestingData(SelectedFeaturesPositions(i),j);
        end
    end
    ...
    TestingFinalPCA.num_data = size(FinalTestingData,2);

    SVMmodel = models{minimum(1)};

    % Testing best SVMModel.
    [ypred] = predict(SVMmodel, TestingFinalPCA.X');

    FinalError = cerror(ypred, TestingFinalPCA.y(:) + 1) * 100;
    ...
    fprintf("SVM says that this sign belongs to class %d" , ypred(1)-1);
end
